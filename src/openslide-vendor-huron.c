...
static bool decode_tile(struct level *l,
  ...
  case HURON_COMPRESSION_JP2K_YCBCR:
    space = OPENSLIDE_JP2K_YCBCR;
    break;
  case HURON_COMPRESSION_JP2K_RGB:
...
static void add_properties(openslide_t *osr, char **props) {
      ...
      if (name) {
        ...
        if (g_str_equal(name, "Resolution") ) {
          ...
          if (new_value != NULL) {
            ...
            g_hash_table_insert(osr->properties,
                                g_strdup("huron.AppMag"),
                                g_strdup(new_value));
          }
          g_strfreev(val_stripped);
        }
  ...
  _openslide_duplicate_int_prop(osr, "huron.AppMag",
                                OPENSLIDE_PROPERTY_NAME_OBJECTIVE_POWER);
  _openslide_duplicate_double_prop(osr, "huron.MPP",
...
static bool huron_open(openslide_t *osr,
  ...
  do {
    ...
    if ((compression != HURON_COMPRESSION_JP2K_YCBCR) &&
        (compression != HURON_COMPRESSION_JP2K_RGB) &&
    ...
    if (TIFFIsTiled(ct.tiff)) {
      ...
      // some Huron slides have some zero-length tiles, apparently due to
      // an encoder bug
      toff_t *tile_sizes;
